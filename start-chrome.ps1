...
foreach ($path in $chromePaths) {
    if (Test-Path $path) {
        Write-Host "Found Chrome at: $path" -ForegroundColor Yellow
...
if (-not $chromeFound) {
    Write-Host "❌ Chrome not found in common locations. Please ensure Chrome is installed." -ForegroundColor Red
...
for ($i = 1; $i -le 10; $i++) {
    try {
        ...
        Write-Host "✅ Chrome is ready on port 9222" -ForegroundColor Green
        $chromeReady = $true
        break
    }
    catch {
...
if (-not $chromeReady) {
    Write-Host "❌ Chrome failed to start or is not responding on port 9222" -ForegroundColor Red
...
if ($args.Count -eq 0) {
    Write-Host "❌ No topic specified" -ForegroundColor Red
...
if (-not (Test-Path "start.js")) {
    Write-Host "❌ start.js not found in current directory" -ForegroundColor Red
...
try {
    ...
    Write-Host "❌ Error starting XBot: $_" -ForegroundColor Red
    exit 1
} 